...
/*   Updated: 2021/07/05 13:53:47 by iltafah          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_tab_action(t_rdline *rdl_vars)
{
	...
	if (rdl_vars->tab_mode == off)
	{
		...
		initialize_vec_content(&rdl_vars->tab_vars.matched_files);
		initialize_vec_of_int(&rdl_vars->tab_vars.files_colors);
		get_matched_files(&rdl_vars->tab_vars);
		...
		print_list_of_matched_files(rdl_vars, &rdl_vars->tab_vars);
